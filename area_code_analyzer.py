    ...
    try:
        ...
        for fila in sheet_ringba.iter_rows(values_only=True):
            if len(fila) >= 4:
                ...
                # Convertir el texto de la campaña y la opción seleccionada a minúsculas
                campaign = campaign.lower()
                campana_seleccionada = campana_seleccionada.lower()

                # Comprobar si la campaña contiene la opción seleccionada
                if "spanish" in campana_seleccionada:
                    if campana_seleccionada.replace(" spanish", "") in campaign:
                        fila_actual += 1
                        sheet_resultado.cell(row=fila_actual, column=1, value=campaign)
                        sheet_resultado.cell(row=fila_actual, column=2, value=caller_id)
                elif campana_seleccionada in campaign:
                    fila_actual += 1
                    sheet_resultado.cell(row=fila_actual, column=1, value=campaign)
                    sheet_resultado.cell(row=fila_actual, column=2, value=caller_id)
